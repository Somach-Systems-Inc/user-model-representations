"""Multi-GPU steering: model sharded with device_map="auto"; the hook moves the
direction to whatever device the target layer lives on. Same math as steer.py.
...
@torch.no_grad()
def reply(model, tokenizer, messages, device, max_new_tokens=600) -> str:
    ...
    ids = enc["input_ids"].to(model.get_input_embeddings().weight.device)
...
def main() -> None:
    ...
    rows = [json.loads(l) for l in open(args.convos) if json.loads(l)["label"] == args.label]
        ...
        for row in tqdm(rows, desc="steering"):
            for kind, d in (("probe", direction), ("random", rand_dir)):
                ...
                for alpha in alphas:
                    ...
                    steerer.alpha = alpha
                    text = reply(model, tokenizer, row["messages"], args.device, args.max_new_tokens)
                    f.flush() if False else None; f.write(json.dumps({
                        "id": row["id"], "direction": kind, "alpha": alpha,
                        "layer": layer, "reply": text,
                    }) + "\n")
    steerer.remove()
